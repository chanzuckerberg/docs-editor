...
function docsConvertFromHTML(
  ...
  const convertedContentState = convertFromHTML(handlers)(safeHTML.html);
...
function getSafeHTML(html: string): SafeHTML {
...
function createTableEntityDataFromElement(
  ...
  while (rr < rowsCount) {
    ...
    while (cc < colsCount) {
      ...
      // html = 'xxx';
      const contentState = docsConvertFromHTML(emptyEditorState, html);
      const id = getEntityDataID(rr, cc);
      data[id] = convertToRaw(contentState);
      cc++;
    }
    rr++;
  }
